...
from .rust_commons import get_type_name, generate_extras, generate_literal_struct_type


def generate_type_aliases(spec: model.LSPModel, types:Dict[str, List[str]])-> Dict[str, List[str]]:
...
def generate_type_alias(alias_def:model.TypeAlias, types:Dict[str, List[str]], spec: model.LSPModel) -> List[str]:
    doc = alias_def.documentation.splitlines(keepends=False) if alias_def.documentation else []
    ...
    if alias_def.type.kind == 'reference':
        ...
        lines += [f"pub type {alias_def.name} = {get_type_name(alias_def.type, types, spec)};"]
        ...
        for sub_type in alias_def.type.items:
            if sub_type.kind == 'base' and sub_type.name == 'null':
                ...
                lines += [f"    ValueType{index}({get_type_name(sub_type, types, spec)}),"]
            index += 1
        lines += ["}", ""]
...
def generate_structures(spec: model.LSPModel, types:Dict[str, List[str]]) -> Dict[str, List[str]]:
    for struct in spec.structures:
        types[struct.name] = generate_struct(struct, types, spec)
    return types

def generate_struct(struct_def:model.Structure, types:Dict[str, List[str]], spec: model.LSPModel) -> List[str]:
    doc = struct_def.documentation.splitlines(keepends=False) if struct_def.documentation else []
